...
from impromptu.ik_solver import IK_ANCHOR_METRIC_COLUMNS, interpolate_anchor_qpos
...
def _metadata(
    *,
    ...
    return {
        "planner": "impromptu_bagatelle_delegated_ik_assignment",
...
def plan_target_keys(
    ...
    *,
    ...
    try:
        ...
        planned = np.asarray(bagatelle_plan.planned_hand_joints, dtype=np.float32).copy()
        if planned.shape[0] != keys.shape[0]:
            raise RuntimeError(
                f"Bagatelle planned_hand_joints has {planned.shape[0]} frames, expected {keys.shape[0]}"
            )
        anchor_frames = (np.arange(keys.shape[0], dtype=np.int64) * substeps).astype(np.int64)
        anchor_solution_qpos = planned.astype(np.float32, copy=True)
        ...
        planned_velocities = np.asarray(bagatelle_plan.planned_hand_velocities, dtype=np.float32).copy()
        ...
        metadata["ik_source"] = "Bagatelle.plan_target_keys"
        metadata["assignment_source"] = "Bagatelle.plan_target_keys"
        metadata["planned_hand_joints_source"] = "Bagatelle.planned_hand_joints"
        metadata["dense_hand_joints_source"] = "interpolated_from_bagatelle_control_frame_qpos"
