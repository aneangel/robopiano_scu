...
from variations.losses.latent_mdn_loss import latent_mdn_total_loss
...
def output_root(config: dict[str, Any]) -> Path:
    ...
    if os.environ.get("VARIATIONS_LATENT_MDN_OUTPUT_ROOT"):
        return Path(os.environ["VARIATIONS_LATENT_MDN_OUTPUT_ROOT"])
...
class MetricsWriter:
    def __init__(self, path: Path) -> None:
        self.path = path
        self.csv_path = ensure_dir(path) / "metrics.csv"
        self.jsonl_path = ensure_dir(path) / "metrics.jsonl"
...
def autoencoder_loss(recon: torch.Tensor, target: torch.Tensor, z: torch.Tensor, cfg: dict[str, Any]) -> dict[str, torch.Tensor]:
    joint_mse = F.mse_loss(recon, target)
    latent_l2 = z.pow(2).mean()
    total = (
        float(cfg.get("joints_weight", 1.0)) * joint_mse
        + float(cfg.get("latent_l2_weight", 1e-4)) * latent_l2
    )
    return {
        "loss": total,
...
def denormalized_pose_metrics(pred_norm: torch.Tensor, target_norm: torch.Tensor, normalizer: HandStateNormalizer) -> dict[str, float]:
    pred = normalizer.denormalize_hand_state(pred_norm)
    target = normalizer.denormalize_hand_state(target_norm)
    diff = pred - target
    return {
        "denorm_joint_mse": float((diff * diff).mean().detach().cpu().item()),
    }
...
def run_autoencoder_epoch(
    ...
    *,
    ...
    model.train(train)
    rows = []
    for batch in tqdm(loader, desc="train ae" if train else "val ae", leave=False):
        hand = batch["hand_state_normalized"].to(device)
        if train:
            optimizer.zero_grad(set_to_none=True)
        recon, z = model(hand)
        losses = autoencoder_loss(recon, hand, z, loss_cfg)
        if train:
        ...
        row.update(denormalized_pose_metrics(recon.detach(), hand, normalizer))
        rows.append(row)
...
def compute_latent_stats(model: torch.nn.Module, loader: DataLoader, device: torch.device) -> dict[str, torch.Tensor]:
    ...
    for batch in tqdm(loader, desc="latent stats", leave=False):
        hand = batch["hand_state_normalized"].to(device)
        chunks.append(model.encode(hand).detach().cpu())
...
def run_mdn_epoch(
    ...
    *,
    ...
    for batch in tqdm(loader, desc="train mdn" if train else "val mdn", leave=False):
        target_keys = batch["target_keys"].to(device)
        hand = batch["hand_state_normalized"].to(device)
        with torch.no_grad():
            z = autoencoder.encode(hand)
            z_true = (z - z_mean) / z_std
        if train:
            optimizer.zero_grad(set_to_none=True)
        mixture_logits, means, log_stds = mdn_model(target_keys)
        losses = latent_mdn_total_loss(
            mixture_logits,
            means,
            log_stds,
            z_true,
            decoder,
            hand,
            mdn_nll_weight=float(loss_cfg.get("mdn_nll_weight", 1.0)),
            pose_aux_weight=float(loss_cfg.get("pose_aux_weight", 0.5)),
        )
        if train:
            losses["loss"].backward()
            if grad_clip_norm is not None and grad_clip_norm > 0:
        ...
        with torch.no_grad():
            ...
            batch_idx = torch.arange(means.shape[0], device=device)
            recon = decoder(means[batch_idx, best_idx])
        row = {key: float(value.detach().cpu().item()) for key, value in losses.items()}
        row.update(denormalized_pose_metrics(recon.detach(), hand, normalizer))
        row["val_score"] = row["nll"] + 0.5 * row["joint_mse"]
        rows.append(row)
    return mean_dict(rows)
...
def maybe_start_wandb(config: dict[str, Any], run_root: Path):
    ...
    try:
        return wandb.init(
            ...
            tags=list(config.get("logging", {}).get("tags", ["variations", "latent-mdn"])),
            job_type="latent_mdn",
            config=config,
            dir=str(run_root),
        )
...
def train_autoencoder(
    ...
    for epoch in range(int(train_cfg.get("epochs", 300))):
        ...
        payload = {
            ...
            "latent_dim": int(config["autoencoder"].get("latent_dim", 12)),
            "input_dim": int(config["autoencoder"].get("input_dim", 46)),
        ...
        if bad_epochs >= patience:
            print(f"Autoencoder early stopping after {bad_epochs} non-improving epochs.")
            break
...
def train_mdn(
    ...
    grad_clip = train_cfg.get("grad_clip_norm")
    grad_clip = float(grad_clip) if grad_clip is not None else None
    for epoch in range(int(train_cfg.get("epochs", 250))):
        ...
        payload = {
            "model_state_dict": model.state_dict(),
            "latent_dim": int(config["mdn"].get("latent_dim", 12)),
            "num_components": int(config["mdn"].get("num_components", 5)),
        ...
        if val_score < best_score:
            ...
            bad_epochs += 1
        if bad_epochs >= patience:
            print(f"MDN early stopping after {bad_epochs} non-improving epochs.")
            break
    return best_path


def main() -> None:
    parser = argparse.ArgumentParser(description="Train Variations latent MDN pipeline.")
    ...
    save_json(run_root / "dataset_summary.json", {"extraction_root": str(extraction_root(config)), "norm_stats": str(norm_path), "train_pairs": len(train_dataset), "val_pairs": len(val_dataset)})
    ...
    if wandb_run is not None:
        ...
        wandb_run.finish()
    print(f"Autoencoder checkpoint: {autoencoder_path}")
    print(f"Latent MDN checkpoint: {mdn_path}")
