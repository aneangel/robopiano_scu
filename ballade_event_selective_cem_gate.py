...
def proxy_tuple(target_keys: np.ndarray, activations: np.ndarray, *, dt: float, threshold: float) -> tuple[float, float, int, int, int]:
    score = score_rollout(
        target_keys=np.asarray(target_keys, dtype=np.float32),
        played_keys=np.asarray(activations, dtype=np.float32),
        dt=float(dt),
    ...
    return (
        event_f1(score),
        float(score.get("frame_f1", 0.0)),
        int(score.get("matched_press_events", 0)),
        int(score.get("mispresses", 0)),
        int(score.get("played_press_events", 0)),
    )
...
def perturb(base: np.ndarray, rng: np.random.Generator, *, finger_sigma: float, forearm_sigma: float) -> np.ndarray:
    ...
    cand[ALL_FINGER_JOINT_INDICES] += rng.normal(0.0, float(finger_sigma), size=ALL_FINGER_JOINT_INDICES.size).astype(np.float32)
    cand[FOREARM_INDICES] += rng.normal(0.0, float(forearm_sigma), size=FOREARM_INDICES.size).astype(np.float32)
    return cand


def main() -> None:
    parser = argparse.ArgumentParser(description="Ballade: event-proxy selective contact CEM for Impromptu.")
    ...
    parser.add_argument("--samples-per-mask", type=int, default=180)
    ...
    parser.add_argument("--forearm-sigma", type=float, default=0.035)
    parser.add_argument("--sigma-decay", type=float, default=0.55)
    parser.add_argument("--fp-rank-weight", type=float, default=0.05)
    parser.add_argument("--min-static-improvement", type=float, default=0.02)
    parser.add_argument("--max-fp-increase", type=float, default=1.0)
    parser.add_argument("--min-proxy-event-improvement", type=float, default=0.002)
    parser.add_argument("--proxy-frame-weight", type=float, default=0.10)
    ...
    candidates_by_mask: dict[tuple[int, ...], tuple[np.ndarray, np.ndarray, tuple[float, int, int, int], dict[str, Any]]] = {}
    baseline_activations = np.zeros((control_qpos.shape[0], 88), dtype=np.float32)
    ...
    with BagatelleKinematics(config=config, target_keys=target_keys, output_dir=out / "kinematics") as kin:
        for frame in range(control_qpos.shape[0]):
            baseline_activations[frame] = kin.activation_for_qpos(control_qpos[frame], settle_steps=1)[:88]
        for key, frames in sorted(masks.items(), key=lambda item: (len(item[0]), item[0])):
            target = target_keys[frames[0]]
            seed_scores: list[tuple[tuple[float, int, int, int], np.ndarray, np.ndarray, str]] = []
            baseline_scores = []
            for frame in frames:
                q = control_qpos[frame].copy()
                activation = baseline_activations[frame]
                score = contact_score(target, activation, args.threshold)
                baseline_scores.append(score)
                seed_scores.append((score, q, activation.copy(), f"frame_{frame}"))
            ...
            centers = [q for _score, q, _activation, _name in candidates]
            ...
            for iteration in range(max(int(args.iterations), 1)):
                ...
                for sample in range(max(int(args.samples_per_mask), 0)):
                    ...
                    cand = kin.clip_qpos(perturb(center, rng, finger_sigma=finger_sigma, forearm_sigma=forearm_sigma))
                    activation = kin.activation_for_qpos(cand, settle_steps=1)[:88]
                    score = contact_score(target, activation, args.threshold)
                    sampled.append((score, cand, activation.copy(), f"iter{iteration}_sample{sample}"))
                ...
                centers = [q for _score, q, _activation, _name in elite]
                finger_sigma *= float(args.sigma_decay)
                forearm_sigma *= float(args.sigma_decay)
            candidates.sort(key=lambda item: rank_score(item[0], fp_weight=args.fp_rank_weight), reverse=True)
            best_score, best_qpos, best_activation, best_source = candidates[0]
            stat_ok = (
                float(best_score[0]) >= base_mean_f1 + float(args.min_static_improvement)
            ...
            if stat_ok:
                candidates_by_mask[key] = (best_qpos.astype(np.float32), best_activation.astype(np.float32), best_score, rows[-1])

    selected: dict[tuple[int, ...], np.ndarray] = {}
    proxy_activations = baseline_activations.copy()
    current_proxy = proxy_tuple(target_keys, proxy_activations, dt=0.05, threshold=args.threshold)
    ordered_keys = sorted(
        candidates_by_mask,
        key=lambda key: (len(masks[key]), candidates_by_mask[key][2][0], candidates_by_mask[key][2][1]),
        reverse=True,
    )
    accepted_rows = []
    for key in ordered_keys:
        _qpos, activation, _score, row = candidates_by_mask[key]
        trial = proxy_activations.copy()
        for frame in masks[key]:
            trial[frame] = activation
        trial_proxy = proxy_tuple(target_keys, trial, dt=0.05, threshold=args.threshold)
        current_value = current_proxy[0] + float(args.proxy_frame_weight) * current_proxy[1]
        trial_value = trial_proxy[0] + float(args.proxy_frame_weight) * trial_proxy[1]
        accept = trial_value >= current_value + float(args.min_proxy_event_improvement)
        accepted_rows.append(
            {
                "keys": list(key),
                "frames": int(len(masks[key])),
                "accepted": bool(accept),
                "proxy_before": list(current_proxy),
                "proxy_after": list(trial_proxy),
                "static_source": row["source"],
            }
        )
        if accept:
            proxy_activations = trial
            current_proxy = trial_proxy
            selected[key] = candidates_by_mask[key][0]

    changed_frames = 0
    for key, best_qpos in selected.items():
        for frame in masks[key]:
            control_qpos[frame] = best_qpos
            changed_frames += 1
    ...
    result = {
        ...
        "output_dir": str(out),
        "masks": int(len(masks)),
        "candidate_masks": int(len(candidates_by_mask)),
        "used_masks": int(len(selected)),
        "changed_control_frames": int(changed_frames),
        "proxy_final_event_f1": float(current_proxy[0]),
        "proxy_final_frame_f1": float(current_proxy[1]),
        ...
        "mask_rows": rows[:300],
        "accepted_rows": accepted_rows[:300],
    }
