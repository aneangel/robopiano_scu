...
def bridge_slice(dense: np.ndarray, start: int, end: int, neutral: np.ndarray) -> np.ndarray:
    count = max(int(end - start), 1)
    left = dense[start - 1] if start > 0 else neutral
    right = dense[end] if end < dense.shape[0] else left
    alpha = np.linspace(0.0, 1.0, count + 2, dtype=np.float32)[1:-1].reshape(-1, 1)
    return ((1.0 - alpha) * left.reshape(1, -1) + alpha * right.reshape(1, -1)).astype(np.float32)


def candidate_slice(mode: str, dense: np.ndarray, start: int, end: int, neutral: np.ndarray) -> np.ndarray:
    if mode == "neutral":
        return np.repeat(neutral.reshape(1, -1), end - start, axis=0).astype(np.float32)
    if mode == "hold_before":
        state = dense[start - 1] if start > 0 else neutral
        return np.repeat(state.reshape(1, -1), end - start, axis=0).astype(np.float32)
    if mode == "hold_after":
        state = dense[end] if end < dense.shape[0] else neutral
        return np.repeat(state.reshape(1, -1), end - start, axis=0).astype(np.float32)
    if mode == "bridge":
        return bridge_slice(dense, start, end, neutral)
...
def is_better(
    ...
    *,
    min_delta_event_f1: float,
    allow_equal_f1_mispress_drop: bool,
) -> bool:
    delta = float(trial["event_f1"] - current["event_f1"])
    if delta >= float(min_delta_event_f1):
        return True
    if not allow_equal_f1_mispress_drop:
        return False
    return delta >= -1e-12 and int(trial["mispresses"]) < int(current["mispresses"])


def main() -> None:
    parser = argparse.ArgumentParser(description="Badinerie: simulator-greedy false-event scrub for Impromptu rollouts.")
    ...
    parser.add_argument("--post-dense-frames", type=int, default=4)
    parser.add_argument("--passes", type=int, default=1)
    parser.add_argument("--max-events-per-pass", type=int, default=120)
    parser.add_argument("--max-total-evals", type=int, default=0)
    parser.add_argument("--modes", default="hold_before,neutral,bridge")
    ...
    max_total_evals = int(args.max_total_evals)

    for pass_index in range(max(int(args.passes), 1)):
        if pass_index > 0:
            current, detail = simulate_score(
                payload,
                dense,
                out / f"eval_pass_{pass_index:02d}_rescore",
                str(args.environment_name),
                float(args.threshold),
                dense_dt,
            )
        mispress_events = list(detail.get("mispress_events", []))[: max(int(args.max_events_per_pass), 0)]
        for event_index, event in enumerate(mispress_events):
            if max_total_evals > 0 and eval_count >= max_total_evals:
                break
            ...
            for mode in modes:
                if max_total_evals > 0 and eval_count >= max_total_evals:
                    break
                dense[start:end] = candidate_slice(mode, dense, start, end, neutral)
                eval_count += 1
                ...
                trial_records.append({"mode": mode, "trial": trial})
                if best_score is None or (
                    float(trial["event_f1"]),
                    -int(trial["mispresses"]),
                    int(trial["matched"]),
                ) > (
                    float(best_score["event_f1"]),
                    -int(best_score["mispresses"]),
                    int(best_score["matched"]),
                ):
                    best_mode = mode
            ...
            if best_score is not None and best_slice is not None and is_better(
                best_score,
                current,
                min_delta_event_f1=float(args.min_delta_event_f1),
                allow_equal_f1_mispress_drop=bool(args.allow_equal_f1_mispress_drop),
            ):
    ...
    payload["badinerie_scrubbed_dense_frames"] = scrubbed
