...
def linear_dense(control: np.ndarray, *, substeps: int) -> np.ndarray:
    rows = np.asarray(control, dtype=np.float32)
    dense_x = np.arange(rows.shape[0] * int(substeps), dtype=np.float64) / float(substeps)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Retest a trajectory with linear dense interpolation from control qpos.")
    ...
    dense = linear_dense(control, substeps=substeps)
