...
def selected_indices(finger: int, scope: str) -> np.ndarray:
    if scope == "full":
        return np.arange(46, dtype=np.int64)
    joints = np.asarray(BAGATELLE_FINGER_JOINT_INDEX_ROWS[int(finger)], dtype=np.int64)
    if scope == "finger":
        return joints
    if scope == "hand":
        return np.unique(np.concatenate([joints, hand_forearm_indices(int(finger))]))
...
def apply_candidate(
    ...
    *,
    ...
    out = dense[start:end].copy()
    cand = candidate_dense[start:end]
    indices = selected_indices(int(finger), str(scope))
    out[:, indices] = cand[:, indices]
    return out.astype(np.float32)


def main() -> None:
    parser = argparse.ArgumentParser(description="Rondo: simulator-greedy injection for missed Impromptu target events.")
    ...
    parser.add_argument("--scopes", default="finger,hand,full")
    ...
    with np.load(args.candidate_npz, allow_pickle=False) as data:
        candidate_payload = {key: np.asarray(data[key]) for key in data.files}
    dense = np.asarray(payload["planned_hand_joints_dense"], dtype=np.float32).copy()
    candidate_dense = np.asarray(candidate_payload["planned_hand_joints_dense"], dtype=np.float32)
    if candidate_dense.shape != dense.shape:
        raise ValueError(f"candidate dense shape {candidate_dense.shape} != source dense shape {dense.shape}")
    target_keys = np.asarray(payload["target_keys"], dtype=np.float32)[:, :88]
    ...
    eval_count = 0
    injected = np.zeros((dense.shape[0],), dtype=np.float32)
    ...
    with BagatelleKinematics(config=config, target_keys=target_keys, output_dir=out / "kinematics") as kin:
        for event_index, event in enumerate(list(detail.get("missed_events", []))[: max(int(args.max_events), 0)]):
            ...
            qpos = candidate_dense[min(max(frame, 0), candidate_dense.shape[0] - 1)]
            ...
            candidate_specs: list[tuple[int, str, dict[str, Any]]] = []
            for item in nearest:
                for scope in scopes:
                    candidate_specs.append((int(item["finger"]), str(scope), item))
            seen: set[tuple[int, str]] = set()
            deduped = []
            for finger, scope, item in candidate_specs:
                dedupe_key = (-1 if scope == "full" else finger, scope)
                if dedupe_key in seen:
                    continue
                seen.add(dedupe_key)
                deduped.append((finger, scope, item))
            for finger, scope, item in deduped:
                if int(args.max_total_evals) > 0 and eval_count >= int(args.max_total_evals):
                    break
                dense[start:end] = apply_candidate(
                    dense,
                    candidate_dense,
                    start=start,
                    end=end,
                ...
                record = {"finger": int(finger), "scope": str(scope), "nearest": item, "trial": trial}
            ...
            matched_gain = int(best_score["matched"] - current["matched"]) if best_score else 0
            accept = bool(best_score) and delta >= float(args.min_delta_event_f1)
            if (
                bool(best_score)
                and bool(args.keep_if_matched_increases)
                and matched_gain > 0
                and float(best_score["event_f1"]) >= float(current["event_f1"]) - max(float(args.matched_gain_max_f1_drop), 0.0)
            ):
                accept = True
            summary = {
    ...
    payload["rondo_injected_dense_frames"] = injected
